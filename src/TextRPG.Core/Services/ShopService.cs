...
public sealed class ShopService
{
    ...
    public void Enter(Player player)
    {
        ...
        // Mapear string a Item del catálogo
        var catalogItem = choice switch
        {
            "Pocion pequena"  => ItemCatalog.PotionSmall,
            "Pocion grande"   => ItemCatalog.PotionLarge,
            "Amuleto de fuerza" => ItemCatalog.AmuletOfStrength,
            "Armadura ligera"   => ItemCatalog.LightArmor,
            _ => null
        };
        ...
        switch (choice)
        {
            ...
            default:
                if (catalogItem != null)
                    _renderer.MarkupLine($"[green]Has comprado {catalogItem.Name}. Se guardó en el inventario.[/]");
                break;
        }

        if (catalogItem != null)
            player.Inventory.Add(catalogItem);
        _renderer.Pause();
    }
}
