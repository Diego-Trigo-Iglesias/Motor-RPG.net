...
public static class EnemySprites
{
    ...
    { for (int dy = 0; dy < h; dy++) for (int dx = 0; dx < w; dx++) s.SetPixel(x + dx, y + dy, c); }

    private static void Set(PixelSprite s, int x, int y, PixelColor c) => s.SetPixel(x, y, c);

    private static PixelColor C(byte r, byte g, byte b) => new(r, g, b);
