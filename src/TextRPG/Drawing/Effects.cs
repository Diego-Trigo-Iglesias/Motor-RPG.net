...
/// animación de viaje 3D con túnel en perspectiva).
...
/// <summary>Efectos visuales: partículas, floating text, flash, shake, animaciones de viaje 3D.</summary>
public sealed class Effects
{
    ...
    // Paletas de colores por localización para la transición
    private static readonly Dictionary<string, (Color Primary, Color Secondary)> LocPalette = new()
    {
        ["village"] = (C(55, 38, 22), C(255, 210, 80)),
        ["forest"]  = (C(10, 22, 6), C(50, 150, 45)),
        ["cave"]    = (C(6, 4, 8), C(60, 140, 230)),
        ["ruins"]   = (C(45, 30, 16), C(255, 200, 70)),
        ["delta"]   = (C(14, 10, 24), C(80, 200, 180)),
        ["dungeon"] = (C(8, 2, 12), C(255, 100, 20)),
    };

    private static Color C(int r, int g, int b, int a = 255) => new((byte)r, (byte)g, (byte)b, (byte)a);

    /// <summary>Inicia la animación de viaje entre dos localizaciones.</summary>
    public void PlayTravel(string fromLoc, string toLoc, float duration = 2.0f)
    {
        _isTraveling = true;
        _travelT = 0;
        _travelDur = duration;
        _travelFromLoc = fromLoc;
        _travelToLoc = toLoc;

        // Generar partículas de transición (estrellas / chispas)
        var rng = Random.Shared;
        for (int i = 0; i < 60; i++)
        {
            float angle = (float)(rng.NextDouble() * MathF.PI * 2);
            float speed = 80 + (float)rng.NextDouble() * 200;
            _parts.Add(new Particle
            {
                X = 480, Y = 300,
                Vx = MathF.Cos(angle) * speed,
                Vy = MathF.Sin(angle) * speed - 50,
                Life = 0.5f + (float)rng.NextDouble() * 1.0f,
                MLife = 1.5f,
                Col = C(200 + rng.Next(55), 200 + rng.Next(55), 200 + rng.Next(55)),
                Sz = 2 + rng.Next(3)
            });
        }
    }
    ...
    public float TravelProgress => _isTraveling ? Math.Clamp(_travelT / _travelDur, 0, 1) : 1f;
    ...
    public void Draw(int sw, int sh)
    {
        ...
        {
            var p = _parts[i];
            float a = Math.Clamp(p.Life / p.MLife, 0, 1);
            DrawCircle((int)p.X, (int)p.Y, p.Sz * a, C(p.Col.R, p.Col.G, p.Col.B, (byte)(a * 200)));
        }

        foreach (var f in _floats)
        {
            float a = Math.Clamp(f.Life / f.MaxLife, 0, 1);
            int sz = f.Critical ? 24 : 18;
            var c = f.Critical ? C(255, 160, 40) : C(225, 230, 240);
            DrawTC(f.Text, (int)f.X, (int)f.Y, sz, C(c.R, c.G, c.B, (byte)(a * 255)));
        }
    }
    ...
    public void DrawTravel(int sw, int sh)
    {
        ...
        // Dibujar partículas (se dibujan automáticamente desde Draw())
    }
    ...
    // CLASES (no structs) para mutación directa en Update()
